...
static ngx_http_module_t ngx_http_mytest_module_ctx = {
    ...
    ngx_http_mytest_merge_loc_conf//merge location configruation
};
...
static ngx_int_t mytest_upstream_create_request(ngx_http_request_t *r)
{
    ngx_log_error(NGX_LOG_ALERT, r->connection->log, 0, "mytest_upstream_create_request" );
    ...
    /*内存池中申请内存, 在网络不佳的情况下，可能需要epoll多次调用send, 这时必须保证这段内存不被释放
    请求结束时, 这段内存会被自动释放, 降低内存泄漏的可能
    */
    ngx_log_error(NGX_LOG_ALERT, r->connection->log, 0, "mytest_upstream_create_request 1" );
    ...
    ngx_log_error(NGX_LOG_ALERT, r->connection->log, 0, "mytest_upstream_create_request 2" );

    b->last = b->pos + queryLineLen;

   ngx_snprintf(b->pos, queryLineLen, (char *)backendQueryLine.data, &r->args);

    //ngx_chain_t结构, 包含要发送给上游服务器的请求
...
static ngx_int_t mytest_process_status_line(ngx_http_request_t *r)
{
    ...
    if (rc == NGX_ERROR)
    {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "upstream sent no valid HTTP/1.0");
...
static ngx_int_t ngx_http_mytest_handler(ngx_http_request_t *r) {
    ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "ngx_http_mytest_handler");


    //建立HTTP上下文结构体ngx_http_mytest_ctx_t
    ...
    /*对每个使用upstream的请求, 必须调用且只能调用1次, 初始化r->upstream成员*/
    ...
    if (pHost == NULL)
    {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, "gethostbyname faild. %s", strerror(errno));
